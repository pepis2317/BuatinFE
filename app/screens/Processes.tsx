...
const ProcessesRoute = ({ navigation }: { navigation: any }) => {
    ...
    const fetchProcesses = async (pageNumber: number) => {
        try {
            ...
            const response = await axios.get(`${API_URL}/get-processes?pageSize=3&pageNumber=${pageNumber}`, {
                headers: {
    ...
    return (
        <FlatList
            ...
            renderItem={({ item }: { item: ProcessResponse }) => <ProcessComponent process={item} navigation={navigation} isSeller={false} />}
...
const RequestsRoute = ({ navigation }: { navigation: any }) => {
    ...
    const fetchRequests = async (pageNumber: number) => {
        try {
            const token = await onGetUserToken!()
            const response = await axios.get(`${API_URL}/get-order-requests?pageSize=3&pageNumber=${pageNumber}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
    ...
    const handleFetch = async (reset = false) => {
        ...
        const result = await fetchRequests(reset ? 1 : page);
        if (!result.error) {
            if (reset) {
    ...
    return (
        <View>
            <FlatList
                ...
                renderItem={({ item }: { item: OrderRequestResponse }) => <OrderRequestComponent request={item} navigation={navigation} respondable={false} />}
                contentContainerStyle={{ paddingBottom: 8 }}
                keyboardShouldPersistTaps="handled"
                onEndReached={loadMore}
                onEndReachedThreshold={0.5}
